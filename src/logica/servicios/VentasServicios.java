...
public class VentasServicios {
    ...
    public void modificaDatosVenta(Venta venta) {
        try {
            PreparedStatement status = conexion.prepareStatement("UPDATE `venta` SET `estado` = ?, `id_empleado_fk` = ? WHERE `venta`.`id_venta` = ?;");

            // Nuevos valores para actualizar
            status.setObject(1, venta.getEstado().toString());
            status.setObject(2, venta.getEmpleado().getId());
            status.setObject(3, venta.getId());
