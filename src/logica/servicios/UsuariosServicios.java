...
public class UsuariosServicios {
    ...
    public ArrayList<Usuario> getUsers() {
        ...
        try {
            ...
            ResultSet rs = status.executeQuery();
            while (rs.next()) {
                resultado.add(new Usuario(rs.getString("nombre"), rs.getString("apellido"), rs.getString("password"), rs.getString("email")));
    ...
    public void insertarUsuarios(String nombre, String apellido, String email, String contrasenia) {

        try {
            PreparedStatement status = conexion.prepareCall("INSERT INTO `usuarios` (`nombre`, `apellido`, `password`, `email`) VALUES (?, ?, ?, ?);");
            status.setString(1, nombre);   
            status.setString(2, apellido); 
            status.setString(3, contrasenia); 
